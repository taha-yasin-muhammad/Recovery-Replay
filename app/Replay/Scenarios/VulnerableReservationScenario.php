...
/**
 * Vulnerable reservation scenario.
 *
 * Each attempt calls the unprotected /api/reservations endpoint in-process.
 * Without idempotency protection, a retry after a 503 creates a second reservation.
 */
class VulnerableReservationScenario implements ReplayScenario
{
    public function label(): string
    {
        return 'reservation (vulnerable)';
    }
    ...
    {
        return $this->dispatch('/api/reservations', [
    ...
    protected function dispatch(string $uri, array $data, string $attemptId): array
    {
        ...
        return [
            ...
            'resource_id' => isset($body['reservation_id']) && is_numeric($body['reservation_id']) ? (int) $body['reservation_id'] : null,
            'order_id' => null,
            'response_body' => $body,
        ];
    }
